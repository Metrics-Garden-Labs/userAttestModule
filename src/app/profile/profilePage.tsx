...
export default function ProfilePage() {
  ...
  const [contributedRepositories, setContributedRepositories] = useState<
    Repository[]
  >([]);
  ...
  const [receivedEndorsements, setReceivedEndorsements] = useState<
    UserEndorsements[]
  >([]);
  const [givenEndorsements, setGivenEndorsements] = useState<
    UserEndorsements[]
  >([]);
  const [endorsementsLoading, setEndorsementsLoading] = useState(true);
  const [endorsementType, setEndorsementType] = useState<"given" | "received">(
    "received"
  );
      ...
      if (githubToken && githubName && activeTab === "repos") {
        try {
          const response = await fetch(
            `/api/github-repos?token=${githubToken}&username=${githubName}`
          );
          if (!response.ok) {
  ...
  const entry: Entry = {
    ...
    emailQuery:
      "[GitHub] A third-party OAuth application has been added to your account from: noreply@github.com",
  ...
  const fetchEndorsements = async () => {
    try {
      ...
      const receivedResponse = await fetch(
        `${NEXT_PUBLIC_URL}/api/getEndorsementsForUser`,
        {
          method: "POST",
          headers: {
      ...
      const givenResponse = await fetch(
        `${NEXT_PUBLIC_URL}/api/getEndorsementsByUser`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 mx-3 lg:gap-8 max-w-6xl overflow-y-auto">
        {endorsementsToRender.map((endorsement, index) => (
          <div
            key={index}
            className="p-4 bg-white border hover:bg-black/5 rounded-lg shadow-md"
          >
          ...
          <div className="px-3 bg-backgroundgray">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 mx-3 lg:gap-8 max-w-6xl overflow-y-auto">
              {repositories.map((repo) => (
                ...
                >
                  ...
                  <a
                    ...
                    className="text-[#E67529] hover:underline"
                  >
                    View Repository
                  </a>
                </div>
              ...
              {contributedRepositories.map((repo) => (
                <div
                  key={repo.id}
                  className="p-4 border hover:bg-black/5 rounded-md shadow-sm bg-white"
                >
                  <h3 className="text-lg font-semibold">{repo.name}</h3>
                  <a
                    href={repo.html_url}
        ...
        return (
          <div className="text-black text-left">
            ...
            <div className="flex space-x-4 mb-4">
              <button
                onClick={() => setEndorsementType("received")}
                className={subTabClasses("received")}
              >
                Received
              </button>
              <button
                onClick={() => setEndorsementType("given")}
                className={subTabClasses("given")}
              >
                Given
              </button>
            </div>
